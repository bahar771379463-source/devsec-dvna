pipeline {  
    ...
    stages {  
        stage('Checkout SCM') { steps { git branch: 'main', url: "${GIT_REPO}", credentialsId: "${GIT_CREDENTIALS}" } }  
        stage('Initialize Trivy Template') { steps { sh '''mkdir -p contrib; curl -sSL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/html.tpl -o contrib/html.tpl''' } }  
        stage('Fetch DockerHub Credentials from Vault') { steps { withVault([vaultSecrets: [[path: 'secret/docker-credentials', secretValues: [[envVar: 'DOCKER_USER', vaultKey: 'username'], [envVar: 'DOCKER_PASS', vaultKey: 'password']]]]]) { sh 'echo $DOCKER_PASS | docker login -u $DOCKER_USER --password-stdin' } } }  

        stage('Check for Code Changes') { steps { script { 
            ...
            if (changes) { echo "🔍 Code changes detected, will build a new image." } else { echo "🟢 No code changes detected." } 
        } } }  
        ...
        stage('Security Scan with Trivy') { steps { script {
            ...
            if (env.VULN_COUNT != "0") {
                ...
                def userChoice = input(id: 'userConfirm', message: "⚠ Trivy اكتشف ${env.VULN_COUNT} ثغرة(ثغرات) HIGH/CRITICAL. هل تريد المتابعة؟", parameters: [[$class: 'ChoiceParameterDefinition', choices: "توقف\nاستمرار", description: 'اختر: توقف لإيقاف الـ pipeline، استمرار لتكملة النشر.', name: 'قرار']])
                if (userChoice == 'توقف') { error("🛑 التوقف بناءً على قرار المستخدم (اكتُشفت ${env.VULN_COUNT} ثغرات).") } else { echo "✅ تم اختيار الاستمرار رغم وجود ${env.VULN_COUNT} ثغرات." }
            } else { echo "✅ No HIGH/CRITICAL vulnerabilities found." }
        } } }

        stage('Push to Docker Hub') { steps { sh "docker push ${IMAGE_NAME}" } }  

        stage('Deploy to Test Server') { steps { sshagent(credentials: ['ssh-test-server']) { sh '''ssh -o StrictHostKeyChecking=no bahar@192.168.1.3 "
        ...
        stage('Smoke Test (Health Check)') { steps { script {
            sleep 5
            def status = sh(script: "curl -o /dev/null -s -w %{http_code} -L http://192.168.1.3:9090", returnStdout: true).trim()
            if (status == "200") { echo "✅ Application is healthy (status: ${status})" } else { error("❌ Application failed health check. Status code: ${status}") }
        } } }
    }
    ...
    post {
        success {
            ...
            emailext(to: "bahar771379463@gmail.com", subject: "✅ Trivy Security Report - Build ${env.BUILD_NUMBER}", body: "Attached is the Trivy and Snyk security scan report for build ${env.BUILD_NUMBER}.", attachmentsPattern: "trivy-report.html, snyk-report.html")
        ...
        failure {
            ...
            emailext(to: "bahar771379463@gmail.com", subject: "❌ Build Failed - Security Scan Report", body: "The build ${env.BUILD_NUMBER} failed. Check Jenkins console for details.", attachmentsPattern: "trivy-report.html, snyk-report.html")
